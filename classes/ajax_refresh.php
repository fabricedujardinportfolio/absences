...
foreach ($list as $rs) {
	...
	// add new option
    echo '<li class="border bg-white" style="list-style-type: none;" onclick="set_item(\''.$rs['name']." ".$rs['first_name'].'\')">'.$name.'</li> ';
}
$pdofn = connect();
	$keyword = '%'.$_POST['keyword'].'%';
	...
	foreach ($listfn as $rsfn) {
		...
		// add new option
		echo '<li class="border bg-white"  onclick="set_item(\''.$rsfn['first_name']." ".$rsfn['name'].'\')">'.$first_name.'</li>';
	}
